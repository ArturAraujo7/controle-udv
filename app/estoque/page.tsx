...
import { ArrowLeft, Beaker, Plus, AlertCircle, Truck, Droplet } from 'lucide-react'

type PreparoComSaldo = {
  id: number
  tipo?: string // Local ou Doação
  data_preparo: string
  mestre_preparo: string
  nucleo_origem?: string // Se for doação
...
export default function GerenciarEstoque() {
  ...
  useEffect(() => {
    async function fetchEstoque() {
      // Busca todos os preparos
      const { data: dadosPreparos } = await supabase
        .from('preparos')
        .select('*')
      ...
      const listaFinal = dadosPreparos?.map((preparo: any) => {
        ...
        // Só soma no total se tiver saldo positivo
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {preparos.map(preparo => {
              ...
              const corTema = isDoacao ? 'blue' : 'green' // Define a cor base (verde ou azul)
