...
export function SeletorMembro({ placeholder = "Selecione ou digite...", value, onChange, membros, onMembroAdicionado }: SeletorMembroProps) {
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      {/* Combobox Trigger */}
      <div 
        className="flex items-center w-full bg-transparent border border-input rounded-lg bg-background cursor-text group focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/50 transition-colors"
        onClick={() => setIsOpen(true)}
      >
        <input
          type="text"
          className="w-full bg-transparent outline-none py-2 px-3 text-sm text-foreground placeholder:text-muted-foreground font-medium"
      ...
      {isOpen && search.length > 0 && (
        <div className="absolute z-50 w-full mt-1 bg-popover border border-border shadow-xl rounded-xl max-h-60 overflow-y-auto overflow-x-hidden flex flex-col">
          {search.length >= 3 ? (
            <>
                ...
                <div className="py-1">
                  {filteredMembros.map(m => (
                    <div
                      ...
                      className={`px-3 py-2 cursor-pointer text-sm flex items-center justify-between hover:bg-accent transition-colors ${value.id === m.id ? 'bg-accent text-accent-foreground font-medium' : 'text-foreground'}`}
                    >
                      <div className="flex flex-col truncate">
                        <span className="truncate">{formatarNome(m.nome_exibicao || m.nome, m.grau)}</span>
                        <div className="flex gap-2 items-center text-[10px] text-muted-foreground font-medium mt-0.5">
                          <span>{m.grau}</span>
                      ...
                      {value.id === m.id && <Check className="w-4 h-4 text-primary flex-shrink-0 ml-2" />}
                    </div>
                  ))}
                </div>
              ) : (
